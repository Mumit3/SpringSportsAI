...
State machine:
    IDLE → PREVIEW → RECORDING → PREVIEW → IDLE (on shutdown)
...
class ZedRecorder:
    ...
    def start_preview(self) -> None:
        """Open ZED in live mode and begin streaming preview frames."""
    ...
    def start_recording(self, svo_path: str, max_seconds: Optional[int] = None) -> None:
        """Queue a recording-start command. Capture thread will pick it up."""
    ...
    def stop_recording(self) -> None:
        """Queue a recording-stop command."""
        with self._cmd_lock:
            self._cmd_record_stop = True

    def shutdown(self, timeout: float = 4.0) -> Optional[str]:
        """Stop recording (if active), close the camera, fully release the handle.

        Must be called before the analytics pipeline opens the SVO file —
        otherwise the SDK refuses to open it (camera handle still held).

        Returns the path of the most recently completed SVO recording, or None.
        """
        with self._cmd_lock:
    ...
    def _open_camera(self) -> "sl.Camera":
        ...
        init.camera_resolution = res_map.get(config.RECORD_RESOLUTION, sl.RESOLUTION.HD2K)
        init.camera_fps        = config.RECORD_FPS
        ...
        print(f"[ZedRecorder] Camera opened ({config.RECORD_RESOLUTION} @ {config.RECORD_FPS} fps)")
        return zed
